function [out,motionInfo] = gbvs_my(img,param,prevMotionInfo)
% modified by yoshida 20180330
...
if ( blurfrac > 0 )
  ...
  master_map_blur = myconv2(myconv2( master_map , k ),k');
  master_map_blur = mat2gray(master_map_blur);
end
% will not use param.unCenterBias 
...
for i = 1 : length(mapnames)
  feat_map = attenuateBordersGBVS(comb_norm_maps{i},4);
  feat_maps{i} = mat2gray(feat_map);
  
  k = mygausskernel( max(size(master_map)) * blurfrac , 2 );
  feat_maps_blur1 = myconv2(myconv2( feat_maps{i} , k ),k');
  feat_maps_blur{i} = mat2gray(feat_maps_blur1);
end

master_map_resized = mat2gray(imresize(master_map,[size(img,1) size(img,2)]));
master_map_blur_resized = mat2gray(imresize(master_map_blur,[size(img,1) size(img,2)]));

out = {};
out.master_map = master_map;
out.master_map_resized = master_map_resized;
out.master_map_blur = master_map_blur;
out.master_map_blur_resized = master_map_blur_resized;
out.top_level_feat_maps = feat_maps;
out.top_level_feat_maps_blur = feat_maps_blur;
